...
class LinkedInStrategy extends OpauthStrategy {
    ...
    public $defaults = array(
        ...
        'profile_fields' => array('id', 'first-name', 'last-name', 'formatted-name', 'headline', 'picture-url',
            'summary', 'location', 'public-profile-url', 'site-standard-profile-request'),
        // From tmhOAuth
        'user_token' => '',
        'user_secret' => '',
        'use_ssl' => true,
    ...
    public function oauth_callback() {
        ...
        if ($_REQUEST['oauth_token'] == $session['oauth_token']) {
            ...
            if ($results !== false && !empty($results['oauth_token']) && !empty($results['oauth_token_secret'])) {
                $profile = $this->_getProfile($results['oauth_token'],
                ...
                if (!empty($profile['id'])) {
                    ...
                    $this->mapProfile($profile, 'site-standard-profile-request.url',
                            'info.urls.linkedin_authenticated');
